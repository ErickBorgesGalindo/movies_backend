...
const addLikedMovie = async (req, res) => {
  const { user_id, movie_id, media_type } = req.body;
  try {
    const result = await pool.query(
      'INSERT INTO "LikedMovies" (user_id, movie_id, media_type) VALUES ($1, $2, $3) RETURNING *',
...
const removeLikedMovieByUserAndMovie = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      res.json({
        message: "Liked movie removed successfully",
        data: result.rows[0],
      });
    } else {
      res
        .status(404)
        .json({ error: "Liked movie not found for this user and movie" });
    }
  } catch (error) {
    res.status(500).json({ error: "Error removing liked movie" });
  }
